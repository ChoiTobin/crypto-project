...
function Candle() {
  ...
  useEffect(() => {
    Apis.CandleFetch('http://localhost:3000/candle').then((data) => {
      setTime(data);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    chartGroup
      ...
      .attr('width', 5)
      .attr('height', (d) =>
        Math.abs(yScaleChart(d.opening_price) - yScaleChart(d.trade_price)),
      )
      // 개시 가격과 종가의 차이의  높이
      .attr('x', (d, i) => i * 7)
      .attr('y', (d) =>
        Math.min(yScaleChart(d.opening_price), yScaleChart(d.trade_price)),
      )
      // 개시 가격과 종가 중 작은 값을 y로 설정
      .style('fill', (d) =>
        d.opening_price < d.trade_price ? 'red' : 'white',
      );
